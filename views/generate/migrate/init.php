...
{
<?php if ($upSql) : ?>
    $builder->execute(<<<'DDL' <?= $upSql ?> DDL); <?php endif ?> <?php if ($insertData) : ?> <?php foreach ($insertData as $tableName => $value) : ?> $builder->batchInsert('<?= $tableName ?>', ['<?= implode("', '", $value['columns']) ?>'], [
