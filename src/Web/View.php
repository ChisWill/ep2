...
final class View extends BaseView
{
    ...
    public function registerJs(string|Script $js, int $position = self::POSITION_END, string $key = null): void
    {
        if (is_string($js)) {
            $js = Html::script($js);
        }
    ...
    public function registerCss(string|Style $css, int $position = self::POSITION_HEAD, string $key = null): void
    {
        if (is_string($css)) {
            $css = Html::style($css);
        }
